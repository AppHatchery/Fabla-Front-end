...
class ReportGenerator {
  ...
  String generateMarkdown(List<SchemaChange> changes, List<ValidationResult> results) {
    final buffer = StringBuffer();
    buffer.writeln('### 🧱 ObjectBox Schema Validation');
    buffer.writeln();
    ...
    if (changes.isEmpty) {
      ...
      int failures = 0;
      buffer.writeln('| Status | Change | Message |');
      buffer.writeln('| :--- | :--- | :--- |');
      ...
      buffer.writeln();
      if (failures == 0) {
        buffer.writeln('**✨ SUCCESS:** Schema is safe for production.');
      } else {
        buffer.writeln('**🛑 FAILURE:** $failures issue(s) detected.');
        buffer.writeln('Please review the failures above and update `audit.yaml` if the changes are intentional.');
      }
    }
