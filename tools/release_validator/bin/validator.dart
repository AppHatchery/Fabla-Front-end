...
void main(List<String> args) async {
  if (args.length < 2) {
    print('Usage: dart bin/validator.dart <base-model.json> <current-model.json> [--markdown <path>]');
    exit(1);
  }
  ...
  try {
    ...
    // 7. Markdown output (optional)
    final markdownIndex = args.indexOf('--markdown');
    if (markdownIndex != -1 && args.length > markdownIndex + 1) {
      final outputPath = args[markdownIndex + 1];
